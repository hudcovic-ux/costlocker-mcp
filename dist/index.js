...
try {
    ...
    process.stderr.write(`Costlocker MCP connected: ${person?.first_name} ${person?.last_name} (${person?.email}), ` +
        `company: ${userInfo.company?.name}, authorization: ${auth}\n`);
}
catch (error) {
    process.stderr.write(`Failed to verify Costlocker credentials: ${error instanceof Error ? error.message : String(error)}\n`);
    process.exit(1);
}
// --- Tool imports ---
import { listProjectsTool, getProjectTool, searchProjectsTool, createProjectTool, updateProjectTool, upsertTasksTool, deleteTaskTool, handleListProjects, handleGetProject, handleSearchProjects, handleCreateProject, handleUpdateProject, handleUpsertTasks, handleDeleteTask, } from './tools/projects.js';
import { logTimeTool, getTimesheetTool, getMonthlyTimesheetTool, getRunningEntryTool, getAssignmentsTool, handleLogTime, handleGetTimesheet, handleGetMonthlyTimesheet, handleGetRunningEntry, handleGetAssignments, } from './tools/timesheet.js';
import { listPeopleTool, getMeTool, getProjectPeopleTool, handleListPeople, handleGetMe, handleGetProjectPeople, } from './tools/people.js';
import { getProjectBudgetTool, getProjectBillingTool, getProjectExpensesTool, handleGetProjectBudget, handleGetProjectBilling, handleGetProjectExpenses, } from './tools/finance.js';
import { listClientsTool, listActivitiesTool, listTagsTool, listGroupsTool, handleListClients, handleListActivities, handleListTags, handleListGroups, } from './tools/lookup.js';
// --- Server setup ---
const server = new Server({ name: 'costlocker-mcp', version: '1.0.0' }, { capabilities: { tools: {}, resources: {}, prompts: {} } });
